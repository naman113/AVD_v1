...
class ThresholdManager:
    ...
    def _setup_engine(self):
        ...
        try:
            ...
            if 'uri' in self.db_config:
                ...
                # Use psycopg3 driver
                uri = f"postgresql+psycopg://{username}:{password}@{host}:{port}/{database}"
                self.engine = create_engine(uri, pool_pre_ping=True)
    ...
    def _should_refresh_cache(self) -> bool:
        ...
        return time.time() - self.last_refresh_time > self.cache_duration
    ...
    def get_thresholds(self, device_id: int, company_id: int = 1) -> Optional[Dict]:
        """Get thresholds for a specific device"""
        if self._should_refresh_cache():
            self._refresh_cache()
        
        # Find all thresholds for this device and company
        device_thresholds = {}
        for (cached_company_id, cached_device_id, parameter), threshold_data in self.thresholds_cache.items():
            if cached_company_id == company_id and cached_device_id == device_id:
                device_thresholds[parameter] = {
                    'min_value': threshold_data['lower_threshold'],
                    'max_value': threshold_data['higher_threshold']
                }
        
        self.logger.info(f"🔍 THRESHOLD LOOKUP for Device {device_id}, Company {company_id}:")
        self.logger.info(f"   Found thresholds: {len(device_thresholds) > 0}")
        if device_thresholds:
            self.logger.info(f"   Threshold data: {device_thresholds}")
        else:
            # Show available keys for debugging
            available_keys = [(cid, did) for cid, did, param in self.thresholds_cache.keys()]
            unique_keys = list(set(available_keys))[:10]
            self.logger.info(f"   Available (company_id, device_id) pairs: {unique_keys}")
        
        return device_thresholds if device_thresholds else None
